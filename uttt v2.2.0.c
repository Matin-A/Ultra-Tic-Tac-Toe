...
int main(){
    ...
    int aWin=0,bWin=0,abest=82,bbest=82;                                                    //abest & bbest are (minimum choices) best record numbers lower than 82(so default number is 82)
    ...
    while (1){                                                                              //whole game runs in an unlimited cycle
        int i,s,t;
        abest=(abest==0)?82:abest;
        bbest=(bbest==0)?82:bbest;
        FILE *fptr;
        fptr=fopen("choices.txt","rt");
        ...
        user_num_i=all2numGet(fptr)-1;                                                     //each input has break with 0 value(user enter 0 but user_num_i will be -1
        ...
        for(i=1;i<82;i++){                                                                 //each round starts here (a draw game will takes 81 turns)
            ...
            if (user_num_i==-1) break;
            user_num_j=all2numGet(fptr)-1;                                                 //get user choices
...
int all2numGet (FILE *fptr)                                                          //scan user input , prevent some error (like user character(s) input)
{
    char user_char[2];
    int user_num;
    do{
        fscanf(fptr,"%c", user_char);
