...
int main()
{
    int i=1,x,isWrong=0,isDuplicate=0;
    static char array[10],a[20],b[20] , ch[2];
    ...
    while (1)                                                                                //game runs in a unlimited cycle
    {
        ...
        while(i<10)                                                                          //each round starts here (a draw game will takes 10 turns)
        {
            ...
            if (i%2==0)                                                                      //will print second player name at his turn like this :    -----name1(+)
            {
                ...
                for(k;k<21 && b[k]!='\0';k++)
                    printf("%c",b[k]);
                 if(isWrong){printf("\n    Incorrect input.Enter Again : ");isWrong=0;}
                 if(isDuplicate){printf("\n    Duplicate input.Enter Again : ");isDuplicate=0;}
            ...
            do{
                scanf("%s" , ch);
                x = ch[0];
                if (ch[1]==NULL && x > 48 && x < 57){
                    x -= 48;
                if((array[x]=='+') || (array[x]=='O')){
                    isDuplicate=1;
                    i--;
                }
                }
